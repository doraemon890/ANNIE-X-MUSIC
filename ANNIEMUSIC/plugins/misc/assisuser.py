...
async def join_userbot(app, chat_id, username):
...
async def unban_and_join_userbot(app, chat_id, username):
    userbot = await get_assistant(chat_id)
...
async def join_group(app, message):
    ...
    if message.chat.username:
        if chat_member.status == ChatMemberStatus.ADMINISTRATOR:
            response = await join_userbot(app, chat_id, message.chat.username)
        else:
        ...
        if chat_member.status == ChatMemberStatus.ADMINISTRATOR:
            ...
            if userbot_member.status in [ChatMemberStatus.BANNED, ChatMemberStatus.RESTRICTED]:
                response = await unban_and_join_userbot(app, chat_id, message.chat.username)
            else:
